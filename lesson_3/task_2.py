...
data_base = PassDataBase("pass_db.sqlite")
login = input("Введите логин: ")
salt, hash = data_base.get_user_data(login)[0]
for i in range(2):
    password = input("Введите пароль: ").encode()
    if salt == "" and hash == "":
        salt = uuid4().hex
        hash = sha256(password + salt.encode()).hexdigest()
        data_base.set_user_data(login, salt, hash)
        print(f'Вы зарегистрировались как {login}.')
